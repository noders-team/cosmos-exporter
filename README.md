...
- `--skip-validator-delegations` - do not collect `cosmos_validator_delegations` at all. **Recommended on Sei.** Delegations are stored keyed by delegator, so selecting them by validator requires scanning the whole delegation store; chains that cap store scans (Sei) refuse the query outright, and on any large chain the per-delegator series are expensive for both the node and Prometheus. If the chain refuses the query, the exporter logs an error and disables this metric for the rest of the process rather than retrying it on every refresh — this flag skips the first attempt too.
